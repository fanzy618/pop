...
func TestConsoleRulesCRUDAndReorder(t *testing.T) {
	...
	var rulesResp rulesListResponse
	if err := json.NewDecoder(resp.Body).Decode(&rulesResp); err != nil {
		t.Fatalf("decode rules: %v", err)
	}
	rulesList := rulesResp.Items

	if len(rulesList) != 2 {
		t.Fatalf("rules length=%d, want 2", len(rulesList))
	}
	if rulesResp.Total != 2 || rulesResp.Page != 1 || rulesResp.PageSize != 20 {
		t.Fatalf("unexpected rules response: %+v", rulesResp)
	}
...
func TestConsoleRulesPaginationAndSearch(t *testing.T) {
	t.Parallel()

	consoleURL, _, _, client := setupConsoleAndProxy(t)

	for i := 1; i <= 25; i++ {
		pattern := "alpha-rule-" + strconv.Itoa(i) + ".test"
		if i%5 == 0 {
			pattern = "beta-rule-" + strconv.Itoa(i) + ".test"
		}
	...
	resp, err := client.Get(consoleURL + "/api/rules?page=2&page_size=10&keyword=beta")
	if err != nil {
		t.Fatalf("GET paged rules: %v", err)
	}
	defer resp.Body.Close()

	var payload rulesListResponse
	if err := json.NewDecoder(resp.Body).Decode(&payload); err != nil {
		t.Fatalf("decode paged rules: %v", err)
	}
	...
	if payload.Page != 1 {
		t.Fatalf("page=%d, want 1 after clamping", payload.Page)
	}
	if payload.PageSize != 10 {
		t.Fatalf("page_size=%d, want 10", payload.PageSize)
	}
	if payload.Keyword != "beta" {
		t.Fatalf("keyword=%q, want beta", payload.Keyword)
	}
	if len(payload.Items) != 5 {
		t.Fatalf("items length=%d, want 5", len(payload.Items))
	}
	for _, rule := range payload.Items {
		if !strings.Contains(strings.ToLower(rule.Pattern), "beta") {
			t.Fatalf("pattern=%q does not contain beta", rule.Pattern)
		}
	}
}
...
func TestConsoleRuleCreateWithUpstreamReference(t *testing.T) {
	...
	var rulesPayload rulesListResponse
	if err := json.NewDecoder(rulesResp.Body).Decode(&rulesPayload); err != nil {
		t.Fatalf("decode rules: %v", err)
	}
...
func TestConsoleCreateRuleOverridesSamePattern(t *testing.T) {
	...
	var rulesPayload rulesListResponse
	if err := json.NewDecoder(resp.Body).Decode(&rulesPayload); err != nil {
		t.Fatalf("decode rules: %v", err)
	}
	rulesList := rulesPayload.Items
	if len(rulesList) != 1 {
...
func setupConsoleAndProxy(t *testing.T) (consoleURL string, proxyURL string, dbPath string, client *http.Client) {
	...
	consoleServer, err := console.NewServer(cfg, db, proxyServer, telStore, nil, proxyServer.Connections())
	if err != nil {
