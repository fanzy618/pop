...
- `telemetry` — `Store` is the bounded ring buffer of activities (capacity + TTL eviction) feeding `/api/activities[/stream]` and `/api/stats`. `Connections` is the in-flight request registry feeding `/api/connections` (live per-flow byte counters, incl. CONNECT/HTTPS tunnels). `SysStatsCollector` periodically samples CPU/mem/goroutines/heap plus deltas from `Store.Snapshot` and exposes them via `/api/stats/history`. Linux-specific reads live in `sysstats_linux.go`; other OSes use `sysstats_other.go`.
